...
### 色・サイズのカスタマイズ方針
背景色・文字色などの色デザイントークンは `lib/template.tsx` 先頭の定数（`BACKGROUND_COLOR` 等）でのみ変更し、実行時のクエリパラメータや環境変数では上書きしない。ランタイムでの色オーバーライド機能は保守負荷に見合わないため持たない方針。

フォントサイズは、タイトルとサイト名をそれぞれクエリパラメータ `titleFontSize` / `siteFontSize` で上書きできる。パラメータ指定時はレスポンシブスケーリングと最小クランプを適用せず、指定値をそのまま実効フォントサイズ（絶対 px）として使用する。省略時のみ既定トークン（`TITLE_FONT_SIZE` / `LABEL_FONT_SIZE`）に短辺基準スケール係数と最小クランプを適用する。

---
_Updated: 2026-06-30 (sync: フォントサイズのクエリパラメータ上書き（titleFontSize / siteFontSize）を許容する方針に変更)_
