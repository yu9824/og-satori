...
- OG Sans JP（Regular / Bold）で日本語を正確に描画（Noto Sans JP をサブセット化・リネームした OFL 準拠フォント）
- 幅・高さ・テキスト幅をクエリパラメータで動的指定
- 環境変数による設定外部化（ブログ名、画像サイズ等）
- デザイントークン（色、フォントサイズ、余白）を `lib/template.tsx` 先頭にまとめて、フォーク時のカスタマイズを容易化
- ローカルプレビュー UI：`/preview` で生成画像を確認、`/preview/config` で色・サイズ等のデザイントークンをブラウザ上で試行（本番 `/api/og` には影響を与えない開発専用ツール）

## Target Use Cases

- ブログ記事の `<meta property="og:image">` として埋め込む OGP 画像の動的生成
- 別ブログへのフォーク：環境変数とデザイントークンを変更するだけで移植可能。`/preview/config` でブラウザ上で配色・寸法を試してから値を確定できる
...
---
_Updated: 2026-06-30 (sync: ローカルプレビュー UI 機能を反映)_
