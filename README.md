...
背景色・文字色・フォントサイズなどの見た目は、[lib/template.tsx](lib/template.tsx) 先頭の「DESIGN TOKENS」ブロックにまとまっています。環境変数やクエリパラメータでは変更できません。該当の定数を編集して再デプロイすると反映されます。
...
| `TITLE_FONT_SIZE` | `56` | タイトルの基準フォントサイズ（px） |
| `LABEL_FONT_SIZE` | `28` | ブログ名ラベルのフォントサイズ（px） |
| `PADDING` | `64` | 外周の余白（px） |
